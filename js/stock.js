...
const StockUI = (() => {
    ...
    return `
      ...
      <div class="stock-table-wrap">
        <table class="stock-table">
          <thead><tr><th>รายการ</th><th>คงเหลือ</th><th>หน่วย</th><th>ปริมาณคงเหลือ</th><th>สถานะ</th><th>ต้นทุน/หน่วย</th></tr></thead>
          <tbody>${mooRows}</tbody>
        </table>
      </div>
  ...
  function _buildSection(all, menu, low, out, type) {
    return Object.entries(all)
      ...
      .map(([id, s]) => {
        ...
        return `
          <tr>
            <td class="stock-name-cell"><span class="s-thumb">${item.img ? `<img src="${item.img}" alt="" onerror="this.outerHTML='${item.emoji}'">` : item.emoji}</span> ${item.name}</td>
            <td class="s-qty ${isOut?'c-red':isLow?'c-orange':'c-green'}">${s.qty}</td>
            <td class="s-unit">${s.unit}</td>
            <td>
              <div class="s-bar-wrap"><div class="s-bar" style="width:${pct}%;background:${barClr}"></div></div>
              <div class="s-bar-label">เตือนเมื่อ ≤ ${s.low} ${s.unit}</div>
            </td>
            <td>${badge}</td>
            <td class="s-cost">฿${s.cost||0}</td>
          </tr>`;
      }).join('');
  }
      ...
      .map(([id, s]) => {
        ...
        return `
          <div class="adj-card ${isOut?'adj-out':isLow?'adj-low':''}">
            ...
            <div class="adj-quick">
              <button class="adj-quick-btn" data-id="${id}" data-delta="-5">−5</button>
              <button class="adj-quick-btn" data-id="${id}" data-delta="5">+5</button>
              <button class="adj-quick-btn" data-id="${id}" data-delta="10">+10</button>
            </div>
            <div class="adj-bot">
              <span class="adj-bot-label">${s.unit} · เตือนเมื่อ ≤</span>
              <input class="adj-low-inp" type="number" min="1" data-id="${id}" value="${s.low}">
              <span class="adj-bot-label">${s.unit}</span>
            </div>
          </div>`;
      }).join('');
    ...
    return `
      <div class="rcv-form">
        <div class="rf-row">
          ...
          <div class="rf-group" style="flex:0 0 160px">
            <label>จำนวนที่รับ</label>
            <div style="display:flex;align-items:center;gap:8px">
              <input id="rcv-qty" type="number" min="1" value="1" class="rf-input" style="flex:1;min-width:0">
              <span id="rcv-unit" style="font-size:14px;color:#64748b;white-space:nowrap;font-weight:600">ชิ้น</span>
            </div>
          </div>
        </div>
        <div style="padding:10px 0 4px">
          <button class="stock-save-btn" id="rcv-btn" style="width:100%">✅ บันทึกรับของ</button>
        </div>
      </div>
  ...
  function _attachEvents(page) {
    if (page === 'adjust') {
      document.querySelectorAll('.adj-btn, .adj-quick-btn').forEach(btn => {
